  ...
  useEffect(() => {
    axios.get('/api/auth/me', { withCredentials: true })
      .then(r => {
        ...
        if (r.data.user) {
          axios.get('/api/internships', { withCredentials: true }).then(resp => setList(resp.data.internships || []))
        }
      })
      .catch(() => {})
  }, [])
  ...
  return (
    <section className="mx-auto max-w-7xl px-4">
      ...
      <div className="mt-4">
        {user ? (
          <>
            <p className="text-gray-700 mb-4">Welcome, <strong>{user.name}</strong></p>
      ...
      {active && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setActive(null)}>
          <div className="bg-white rounded-xl p-6 max-w-2xl w_full mx-4" onClick={e => e.stopPropagation()}>
